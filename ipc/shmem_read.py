...
class ShmemReader12:
    ...
    def __init__(self, shname, w = 1936, h = 1216):
        ...
        self.buf = np.ndarray([self.size,], dtype=np.uint8, buffer=self.shmem.buf)
    ...
    def parse(self):
        ...
        for i in range(0, self.size, 3):
                self.data[i_data] = np.uint16(self.buf[i]) << 4 +  ( (self.buf[i+1] & 0xF0) >> 4)
                i_data+=1
                self.data[i_data] = (self.buf[i+2]) +  np.uint16(self.buf[i+1] & 0x0F) << 8
                i_data+=1
        self.data = self.data.reshape([self.h, self.w])
